...
def order_points(pts: np.ndarray):
    # center of the 4 coords
    center = np.mean(pts, axis=0)
    
    # average of each point relative to center
    angles = np.arctan2(pts[:, 1] - center[1], pts[:, 0] - center[0])
    
    # sort the points to get clockwise order
    sorted_indices = np.argsort(angles)
    sorted_pts = pts[sorted_indices]
    
    # find top-left point by taking distance from (0,0)
    dists = np.linalg.norm(sorted_pts, axis=1)
    tl_index = np.argmin(dists)
    
    # make the top-left point first
...
def process_image(image, detector: cv2.aruco.ArucoDetector, args):
    """
    ...
    if num_markers not in [1, 2, 4] or (args.strict > 0 and args.strict != num_markers):
        if ids is not None:
    ...
    if num_markers == 4:
        ...
        cv2.aruco.drawDetectedMarkers(annotated, corners, ids)
        for pt in ordered_outer_corners: 
        ...
        # average the markers for better persp corr results
        ...
        # transform all 8 corners based on averaged marker
        all_corners = np.vstack((m1_corners, m2_corners))
        corners_to_transform = all_corners.reshape(-1, 1, 2)
        transformed_corners = cv2.perspectiveTransform(corners_to_transform, H_temp).squeeze()

        # find the outermost bounds of both markers in corrected img
        ...
        # shift the destination average marker points to account for our crop
...
def main():
    ...
    parser.add_argument("-s", "--strict", type=int, default=-1, help="Only count four-corner detections as success")
